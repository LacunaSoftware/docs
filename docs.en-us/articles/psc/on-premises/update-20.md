# Update Lacuna PSC from 1.x to 2.0

Now that ASP.NET Core 6.0 is deprecated, version [2.0](../changelog.md#v2-0-0) of [Lacuna PSC](../index.md) has been migrated to use ASP.NET Core 8.0, which is
...
```
Application starting: PSC (version: *, spaVersion: *, appDiscriminator: 'YOUR_APP_DISCRIMINATOR')
                                                                         ^^^^^^^^^^^^^^^^^^^^^^
```
...
If your instance is hosted on Azure App Services:
